...
function create(req, res, next) {
  ...
  Menu.findOne({ where: { name: req.body.name } })
    .then((data) => {
      if (data)
        //если данные есть вернётся reject в then и нас кинет в ветку catch
        return Promise.reject({ statusCode: 422, message: "нету данных" });
      //можно и объект вернуть
      else {
...
###***объект sequelize имеет методы которые иногда используются в сочетании с моделью и таблицей 
   ...
   fn('COUNT', sequelize.col('name'), 'new') - это функция которая передаёт все аргументы той функции
                                              которую мы указали 1м аргументом.
                                              1. что за функция, 2. какую колонку взять, 3. как назвать поле в которое положить результат
